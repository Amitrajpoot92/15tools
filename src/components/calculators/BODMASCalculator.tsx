...
Calculate Online: https://topcalcbox.com/bodmas-calculator/`;
          ...
          <p className="text-sm text-amber-800/70 mb-2 uppercase tracking-widest font-bold">Result</p>
          <motion.div 
            key={result}
            initial={{ scale: 0.8, opacity: 0 }}
            animate={{ scale: 1, opacity: 1 }}
            className={`text-4xl md:text-5xl font-extrabold tracking-tighter drop-shadow-sm flex items-end gap-1 ${result === 'Error' ? 'text-red-700' : 'text-slate-900'}`}
          >
            {result}
          </motion.div>
        </div>
