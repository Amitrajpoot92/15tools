      ...
      <div className="space-y-5">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div className="space-y-2">
            <label className="text-xs font-bold text-slate-900 uppercase tracking-wider">Original Price ({currency})</label>
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <label className="text-xs font-bold text-slate-900 uppercase tracking-wider">Discount</label>
              <div className="flex items-center bg-slate-50 p-1 rounded-lg border border-slate-200">
                <button 
                  onClick={() => setDiscountType("percent")}
                  className={`px-2.5 py-0.5 text-[10px] sm:text-xs font-bold rounded-md transition-all ${discountType === "percent" ? "bg-white text-indigo-600 shadow-sm border border-slate-200/50" : "text-slate-900 hover:text-slate-700"}`}
                >
                  % Off
                </button>
                <button 
                  onClick={() => setDiscountType("flat")}
                  className={`px-2.5 py-0.5 text-[10px] sm:text-xs font-bold rounded-md transition-all ${discountType === "flat" ? "bg-white text-indigo-600 shadow-sm border border-slate-200/50" : "text-slate-900 hover:text-slate-700"}`}
                >
                  Flat {currency}
                </button>
              </div>
            ...
            <div className="relative">
              {discountType === "flat" && <span className="absolute left-4 top-1/2 -translate-y-1/2 text-lg text-slate-800 font-bold">{currency}</span>}
              <input 
                type="number" 
                value={discount} 
              ...
              />
              {discountType === "percent" && <span className="absolute right-4 top-1/2 -translate-y-1/2 text-lg text-slate-800 font-bold">%</span>}
            </div>
              ...
              {discountType === "percent" && (
                ...
                >
                  {[10, 15, 20, 30, 50, 70].map(v => (
                    <button key={v} onClick={() => setDiscount(v.toString())} className="px-2.5 py-1 bg-slate-100 hover:bg-slate-200 text-slate-900 text-[11px] font-bold rounded-md transition-colors">
                      {v}%
                    </button>
                  ))}
                </motion.div>
        ...
        <div className="space-y-2">
          <label className="text-xs font-bold text-slate-900 uppercase tracking-wider">
            Additional Coupon Discount (%) <span className="text-slate-800 font-medium normal-case">(Optional)</span>
          </label>
          <div className="relative w-full sm:w-1/2 pr-0 sm:pr-2">
            ...
            />
            <span className="absolute right-4 sm:right-6 top-1/2 -translate-y-1/2 text-base text-slate-800 font-bold">%</span>
          </div>
        </div>

        {/* Result Box */}
        <div className="bg-emerald-50 border border-emerald-200 rounded-2xl p-5 md:p-6 mt-5 relative overflow-hidden">
          ...
          <div className="flex flex-wrap items-center gap-3 mb-5">
            ...
            {parseFloat(res.savePercent) > 0 && (
              <div className="px-2.5 py-1 bg-emerald-100 text-slate-900 rounded-md font-bold text-xs">
          ...
          <div className="grid grid-cols-2 gap-4 pt-3 border-t border-emerald-200/50">
            ...
            <div>
              <p className="text-[10px] font-bold text-slate-900 mb-0.5 uppercase tracking-wider">You Save</p>
              <p className="text-sm font-bold text-slate-900">-{currency}{res.totalSaved}</p>
            </div>
          </div>
        </div>
      </div>
